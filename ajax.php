...
class AuthorizationAjaxRequest extends AjaxRequest
{
    ...
    public function register()
    {
        ...
        $ips = $this->getRequestParam("ips"); 
        $vremya = $this->getRequestParam("vremya"); 
        ...
        $ips = htmlspecialchars(trim($ips));
        $vremya = htmlspecialchars(trim($vremya)); 
        $user = new Auth\User();

        try {
            $new_user_id = $user->create($username, $names, $ips, $vremya, $password1);
        } catch (\Exception $e) {
